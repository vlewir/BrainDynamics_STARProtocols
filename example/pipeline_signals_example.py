...
## 3bis. Plot signals example from one Control trial
...
i, j = 0, 1 # channels
OFFSET = -20 # offset between signal amplitudes (for better visibility)
y_ticks = []
for k in range(data.chan_nr):
    if k == i or k == j:
        linestyle = "--"
    else:
        linestyle = "-"
    if k % 2 == 0:
        ...
        color = "red"
    ax.plot(control_trial_samps[0][k] + k*OFFSET, color=color, ls=linestyle)
    y_ticks += [k*OFFSET]

y_ticklabels = data.info_dict["chan_name_list"]

ax.set_xlabel("time [S.U.]")
ax.set_ylabel("ROI", labelpad=10)
ax.set_yticks(y_ticks[0::2], y_ticklabels[0::2])

rhs_ax = ax.twinx()
rhs_ax.set_yticks(y_ticks[1::2], y_ticklabels[1::2])
rhs_ax.set_ylim(ax.get_ylim())

plt.savefig(os.path.join(OUTPUT_DIR, "signals_example.png"), bbox_inches='tight', dpi=300)
plt.savefig(os.path.join(OUTPUT_DIR, "signals_example.pdf"), bbox_inches='tight', dpi=300)
...
## 4bis. Plot SCA example between two channels
fig, ax = plt.subplots(figsize=(10, 7))

corrgram.compute(control_trial_samps[0][0], control_trial_samps[0][1]) # two channels
_ = corrgram.get_maxabs()

corrgram.plot(show_maxabs=True, show_interp=True)
ax.set_ylabel("scaled correlation")
ax.set_ylim([-1.0, 1.0])
plt.savefig(os.path.join(OUTPUT_DIR, "SCA_example.png"), bbox_inches='tight', dpi=300)
plt.savefig(os.path.join(OUTPUT_DIR, "SCA_example.pdf"), bbox_inches='tight', dpi=300)
